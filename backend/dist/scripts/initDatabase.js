...
async function runMigration() {
    ...
    `);
        ...
        const usuariosPath = path_1.default.join(__dirname, "../data/usuarios.json");
        const produtosPath = path_1.default.join(__dirname, "../data/produtos.json");
        const pedidosPath = path_1.default.join(__dirname, "../data/pedidos.json");
        // Migrate Usuários
        if (fs_1.default.existsSync(usuariosPath)) {
            const usuarios = JSON.parse(fs_1.default.readFileSync(usuariosPath, "utf-8"));
            ...
            for (const usuario of usuarios) {
                ...
                if (usuario.role === "employee" || usuario.role === "admin") {
                    await connection.execute(`INSERT INTO usuarios (id, nome, email, senha, telefone, funcao, id_restaurante, ativo, criado_em)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                        usuario.id,
                        usuario.name,
                        usuario.email,
                ...
                // Inserir clientes na tabela clientes
                    ...
                    await connection.execute(`INSERT INTO clientes (id, nome, email, senha, telefone, ativo, criado_em)
             VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                        usuario.id,
                        usuario.name,
                        usuario.email,
       ...
       ('cat-2', 'Bebidas', 'Bebidas variadas', 'rest-1', true)`);
           ...
           VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                ...
                if (produto.variations && Array.isArray(produto.variations)) {
                    for (const variation of produto.variations) {
                        await connection.execute(`INSERT INTO variacoes (id, id_produto, nome, preco)
               VALUES (?, ?, ?, ?)`, [variation.id, produto.id, variation.name, variation.price]);
                    }
                }
                // Inserir adicionais
                if (produto.addons && Array.isArray(produto.addons)) {
                    for (const addon of produto.addons) {
                        await connection.execute(`INSERT INTO adicionais (id, id_produto, nome, preco)
               VALUES (?, ?, ?, ?)`, [addon.id, produto.id, addon.name, addon.price]);
                    }
                }
            }
        ...
        // Migrate Pedidos
        if (fs_1.default.existsSync(pedidosPath)) {
            const pedidos = JSON.parse(fs_1.default.readFileSync(pedidosPath, "utf-8"));
            ...
            for (const pedido of pedidos) {
                ...
                const deliveryAddressStr = pedido.deliveryAddress
                    ...
                    : null;
                try {
                    await connection.execute(`INSERT INTO pedidos (id, id_cliente, nome_cliente, telefone_cliente, id_restaurante, nome_restaurante, metodo_pagamento, subtotal, taxa_entrega, desconto, total, status, endereco_entrega, criado_em, atualizado_em)
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`, [
                        pedido.id,
                        pedido.customerId,
                    ...
                    if (pedido.items && Array.isArray(pedido.items)) {
                        for (const item of pedido.items) {
                            await connection.execute(`INSERT INTO itens_pedido (id_pedido, id_produto, nome_produto, id_variacao, nome_variacao, quantidade, subtotal)
                 VALUES (?, ?, ?, ?, ?, ?, ?)`, [
                                pedido.id,
                                item.productId,
